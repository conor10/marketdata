...
class TestStandardAndPoors(unittest.TestCase):
    def test_parse_response_sp500(self):
        with open('test/data/standardandpoors/SP500ConstituentsExport.xls', 'r') \
                as f:
            symbols = sp.parse_response(f)
        self.assertEqual(501, len(symbols))

    def test_parse_response_asx200(self):
        with open('test/data/standardandpoors/ASX200ConstituentsExport.xls', 'r') \
                as f:
            symbols = sp.parse_response(f)
    ...
    def test_parse_changed_format(self):
        with open('test/data/standardandpoors/SP500ChangedConstituentsExport.xls',
                  'r') as f:
